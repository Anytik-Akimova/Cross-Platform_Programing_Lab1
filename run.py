...
import sys  # for testing if stdin is a terminal
...
def xvm_debug():
    ...
    while True:
        # for testing if stdin is a terminal
        try:
            # Тільки в інтерактивному режимі показуємо запит "xvm > "
            prompt = "xvm > " if is_interactive else ""
            line = input(prompt).strip() 
        except (EOFError, KeyboardInterrupt):
            # Якщо досягнуто кінця файлу в пайпі, або натиснуто Ctrl+C/D
            if is_interactive: # Якщо користувач інтерактивно вийшов
                print("\nDebug was stopped")
            return # Виходимо, щоб уникнути подвійного запиту
        ...
        #run
            ...
            try:
                ...
                print(f"\nStopped at instruction {vm.ip} (BREAKPOINT)")
                vm.ip += 1 
            except IndexError:
        ...
        #step        
            ...
            if not code:
                print("\nNo program loaded. Use: load <path>")
                continue
            if vm.ip >= len(code[vm.current_frame]['instructions']):
                print("\nIP at end; nothing to execute.")
                continue
            ...
            if op.opcode == OpCode.BREAKPOINT:
                ...
                if op.opcode == OpCode.INPUT_NUMBER:
                    ...
                    print("\nInput for INPUT_STRING (one arg): ")
                    inp = input()
                    io = MyIO(inp)
                    vm.input_fn = io.input_fn
                    vm.print_fn = io.print_fn
        ...
        #next
            ...
            #and then run full function code
                ...
                #execute call
                print(f"\n{vm.ip:04d}: {repr(op)}  # CALL (step over)")
                vm.run_op(op, code[vm.current_frame]['labels'])
                vm.ip += 1
            ...
            try:
                if(len(parts) == 1):
                    ...
                    if last < 0 or last > len(list_stack):
                        print(f"\nIndex out of range (0..{len(list_stack)})")
                        continue
                    ...
                    if not slice_:
                        ...
                        continue
                    print("\nStack slice (top is [0]):")
                    for i, v in enumerate(slice_, start=0):
                        print(f"[{i}] {repr(v)}")               
                else:
                    print("\nIncorrect input. Usage: stack [N]")
            except ValueError:
        ...
        #memory
            ...
            if not variables_map:
                print(f"\nFrame {vm.current_frame} has no variables")
            else:
                print(f"\nFrame {vm.current_frame} variables:")
                for k, v in variables_map.items():
                    print(f"  {k} = {repr(v)}")

            if vm.frames:
                if len(vm.frames) != len(vm.saved_ips):
                    raise Exception(f"\nFrames are handled incorrectly!")
                for i in range(len(vm.frames)):
                    if not vm.frames[i].items():
                        print(f"\nFrame {vm.saved_ips[i][0]}' memory is empty\n")
                        continue
                    print(f"\nFrame {vm.saved_ips[i][0]} variables")
        ...
        #exec
            ...
            parts = arg.split()
            if not parts:
                print("\nUsage: exec <OPCODE> <arg1, arg2, ..., argN>")
                continue
        ...
        #frame
            ...
            if(len(current_func) == 0): 
                print("\nThere are no active frames")
                continue

            print(f"Active frame: {current_func}")
            if not variables_map:
                print("\n Current frame has no variables")
                continue
