    ...
    while True:
        ...
        # ---- run ----     
            ...
            try:
                while True:
                    op = code[ip]
                    vm.ip = ip     # Cинхронізація з VM
                    vm.run_op(op, labels)
                    ip = (vm.ip + 1) if vm.ip != ip else (ip + 1) # Cинхронізація з VM
        ...
        # виконуєш її повністю і зупиняєшся на наступній інструкції після виклику
            ...
            try:
                ...
                ip = (vm.ip + 1) if vm.ip != start_ip else (ip + 1)
...
def step_instruction(vm, ip, code, labels):
    ...
    try:
        ...
        return (vm.ip + 1) if vm.ip != ip else (ip + 1) # Синхронізація з VM
